ReactiveTableCounts = new Mongo.Collection("reactive-table-counts");
...
var updateHandle = function (set_context) {
    ...
    if (context.server) {
        ...
        var filterFields = getFilterFields(filters, context.fields);
        var filterValues = getFilterStrings(filters);

        var reactiveTableInfo = {
            tableId: publicationId,
            rows: rowsPerPage,
            page: currentPage,
            index: currentIndex,
            sort: sortQuery,
            options: _.clone(options),
            filterQuery: getFilterQuery(filterValues, filterFields, options)
        }

        ReactiveTable.tableInfo = ReactiveTable.tableInfo || {};
        ReactiveTable.tableInfo[publicationId] = reactiveTableInfo;

        // Session.set("reactive_table_info", reactiveTableInfo);
        // console.log("updateHandle: rows: ", rowsPerPage, ", page: ", currentPage, ", index: ", currentIndex, ", sort: ", sortQuery, "filters: ", filters);


        var onReady = function () {
            if (publicationId === context.nextPublicationId.get()) {
                oldID = context.publicationId.get()
                // console.log("onReady!", context);
                // console.log("deleting the old table info: ", oldID);
                if(oldID)
                    delete ReactiveTable.tableInfo[oldID];
...
var setup = function () {
    ...
    // console.log("reactive-table startup: ", context);
};
...
var getPageCount = function () {
    ...
    // console.log ("getPageCount: ", this.server);
    if (this.server) {
...
Template.reactiveTable.helpers({
    ...
    'getField': function (object) {
        // console.log("getField: ", this, object);
        var fn = this.render || this.fn || function (value) { return value; };
    ...
    'sortedRows': function () {
        if (this.server) {
            ...
            if (!sortKeyField) {
                ...
                console.log ("reactiveTable query: ", filterQuery, {
                    sort: sortQuery,
                    skip: skip,
                    limit: limit
                });
...
Template.reactiveTable.events({
    'click .reactive-table .sortable': function (event) {
        ...
        // console.log("settings: ", template.data.settings);
        // console.log("context: ", template.context);
        if(template.data.settings && template.data.settings.updateCallback){
            // console.log("calling updateCallback");
            template.data.settings.updateCallback.call(template, {sortDirection: template.context.sortDirection.get(), sortKey: template.context.sortKey.get()});
        }
    ...
    'change .reactive-table-navigation .rows-per-page input': function (event) {
        ...
        if(template.data.settings && template.data.settings.updateCallback){
            // console.log("calling updateCallback");
            template.data.settings.updateCallback.call(template, {rowsPerPage: template.context.rowsPerPage.get()});
        }
        getUpdateHandleForTemplate(template)(template.context);
    },
    ...
    'change .reactive-table-navigation .page-number input': function (event) {
        ...
        if(template.data.settings && template.data.settings.updateCallback){
            // console.log("calling updateCallback");
            template.data.settings.updateCallback.call(template, {currentPage: template.context.currentPage.get()});
        }
        getUpdateHandleForTemplate(template)(template.context);
    },
