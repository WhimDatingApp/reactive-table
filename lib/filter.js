...
getFilterQuery = function (filterInputs, filterFields, settings) {
  console.log("getFilterQuery: ", filterInputs, filterFields, settings);
  settings = settings || {};
  ...
  _.each(filterInputs, function (filter, index) {
    ...
    if (filter) {
      if (_.isObject(filter) && _.has(filter, '$nor')){
        // console.log("it's a $nor");
        queryList.push(filter);
      }
      else if (_.isObject(filter) || typeof(filter) == 'number') {
        ...
        if (fieldQueries.length) {
          ...
          // console.log ("fieldQueries: ", fieldQueries, queryList);
      } else {
...
if (Meteor.isClient) {
  ...
  ReactiveTable.listFilters = function () {
    return reactiveTableFilters;
  }
